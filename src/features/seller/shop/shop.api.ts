import type { SellerIdentity } from "../shared/seller.format";
 ...
 *
 * ชื่อร้านใช้ชื่อที่แสดงของผู้ขายที่ login อยู่ — โปรไฟล์ผู้ใช้คือหน้าร้าน [RQ-2]
 * ชื่อร้านแยกต่างหากยังรอข้อสรุปใน SLR-05
 */
function shopMock(seller: SellerIdentity): ShopData {
  return {
    title: "จัดการร้านค้า",
...
/**
 * จุดต่อข้อมูลของหน้าจัดการร้านค้า — ตอนนี้คืนข้อมูลจำลอง ยกเว้นชื่อผู้ขาย
 * วันที่ต่อ API จริงให้แก้เฉพาะข้างในฟังก์ชันนี้ component ทุกตัวไม่ต้องแตะ
 */
export function getShopData(seller: SellerIdentity): ShopData {
  return shopMock(seller);
}
